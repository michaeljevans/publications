...
app.post('/api/v1/papers', (request, response) => {
  ...
  for (let requiredParameter of ['title', 'author']) {
    if (!paper[requiredParameter]) {
      return response
        .status(422)
        .send({ error: `Expected format: { "title": <String>, "author": <String> }. Missing "${requiredParameter}" property.` });
    }
  }
...
app.post('/api/v1/footnotes', (request, response) => {
  ...
  for (let requiredParameter of ['note', 'paper_id']) {
    if (!footnote[requiredParameter]) {
      return response
        .status(422)
        .send({ error: `Expected format: { "note": <String>, "paper_id": <Integer> }. Missing "${requiredParameter}" property.` });
    }
  }
